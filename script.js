...
function setupMobileEventListeners() {
  ...
  function resetLocationChoiceBlock() {
    if (locationChoiceBlock) locationChoiceBlock.style.display = 'block';
    if (currentLocStatusMsg) currentLocStatusMsg.textContent = '';
    if (deliveryShowManualLocBtn) deliveryShowManualLocBtn.style.display = 'none';
    if (manualLocationFields) manualLocationFields.style.display = 'none';
    if (manualDeliveryAddress) manualDeliveryAddress.value = '';
    locationObj = null;
    usingManualLoc = false;
    if (deliveryDistanceDisplay) deliveryDistanceDisplay.style.display = 'none';
  }
  ...
  function showOrHideLocationBlock() {
    ...
    if (orderType.value === 'Delivery') {
      ...
      if (locationChoiceBlock) locationChoiceBlock.style.display = 'none';
      if (deliveryDistanceDisplay) deliveryDistanceDisplay.style.display = 'none';
      locationObj = null;
      usingManualLoc = false;
    }
  ...
  if (deliveryShareLocationBtn) {
    deliveryShareLocationBtn.onclick = function () {
      if (!currentLocStatusMsg || !distanceText || !deliveryDistanceDisplay) return;
      ...
      if (!navigator.geolocation) {
        ...
        if (deliveryShowManualLocBtn) deliveryShowManualLocBtn.style.display = 'block';
        return;
      }
  ...
  if (deliveryShowManualLocBtn) {
    deliveryShowManualLocBtn.onclick = function () {
      usingManualLoc = true;
      if (manualLocationFields) manualLocationFields.style.display = 'block';
      if (currentLocStatusMsg) currentLocStatusMsg.textContent = '';
      if (distanceText) distanceText.textContent = "Distance: Calculating...";
      if (deliveryDistanceDisplay) deliveryDistanceDisplay.style.display = 'block';
  ...
  // Enhanced place order button with touch feedback
  if (placeOrderBtn) {
...
function updateLocationFromMarker() {
  ...
  fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${position.lat}&lon=${position.lng}`)
    ...
    .then(data => {
      if (data.display_name && document.getElementById("manualDeliveryAddress")) {
  ...
  calculateRoadDistance(position.lat, position.lng, RESTAURANT_LOCATION.lat, RESTAURANT_LOCATION.lng)
    .then(distance => {
      deliveryDistance = distance;
      if (distanceText) distanceText.textContent = `Distance: ${distance.toFixed(1)} km`;
      updateCart();
      checkDeliveryRestriction();
    })
    .catch(error => {
      ...
      if (distanceText) distanceText.textContent = `Distance: ${deliveryDistance.toFixed(1)} km (straight line)`;
      updateCart();
      checkDeliveryRestriction();
    });
}
...
async function checkDeliveryRestriction() {
  if (!locationObj || !deliveryRestriction) {
    if (deliveryRestriction) deliveryRestriction.style.display = 'none';
    return;
  }
...
function setupRatingSystem() {
  ...
  if (stars && ratingFeedback && submitBtn) {
    stars.forEach(star => {
...
async function updateCart() {
  ...
  if (orderType === 'Delivery' && locationObj) {
    ...
    if (deliveryDetails && deliveryChargeDisplay) {
      let deliveryMessage = "";
  ...
  if (mobileClearCartBtn && mobileCheckoutBtn) {
    if (itemCount > 0) {
...
function showOrderConfirmationModal(orderData) {
  ...
  const orderSummary = document.getElementById('orderConfirmationDetails');
