// menu.js - Updated to handle category parameter from URL
...
async function initMenu() {
  try {
    ...
    // NEW: Check if a category was specified in the URL
    const params = getUrlParams();
    if (params.category) {
      // Find and activate the specified category tab
      const categoryTab = document.querySelector(`.mobile-category-tab[data-category="${params.category}"]`);
      if (categoryTab) {
        document.querySelector('.mobile-category-tab.active')?.classList.remove('active');
        categoryTab.classList.add('active');
        renderMenuItems(params.category);
        
        // Scroll to the category section
        setTimeout(() => {
          categoryTab.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }, 300);
      }
    } else {
      // Default behavior if no category specified
      renderMenuItems();
    }
...
// Rest of the menu.js code remains the same...
