  ...
  try {
    ...
    if (response.failureCount > 0) {
      response.responses.forEach((resp, idx) => {
        if (!resp.success) removeInvalidToken(tokens[idx]);
      });
    }
  } catch (error) {
...
function playNewOrderSound() {
  const notificationSound = new Audio('https://assets.mixkit.co/active_storage/sfx/989/989-preview.mp3');
  ...
  if (user && user.email === ADMIN_EMAIL) {
    try {
      ...
      messaging.onMessage((payload) => {
        ...
        showAdminNotification(payload.notification?.body || 'New message');
      });
    } catch (error) {
