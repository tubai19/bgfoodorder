...
async function saveOrderToDatabase() {
  ...
  const orderData = {
    orderId: currentOrderId, // Store the numeric order ID
  ...
  try {
    // Create a document with the order ID as the document ID
    await setDoc(doc(db, "orders", currentOrderId.toString()), orderData);
    return true;
  } catch (error) {
